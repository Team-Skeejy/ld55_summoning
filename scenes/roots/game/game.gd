...
func spawn_entity_by_name(entity_name: String) -> void:
	var property: Property = PropertyFactory.entity_by_name(entity_name)
...
func spawn_second_entity() -> bool:
	var roll: float = randf_range(0, 1)
	if roll < 0.5:
		return false
	var property: Property = PropertyFactory.get_second_dropped_property()
	var entity: Entity = test_entity.instantiate()
	add_child(entity)
	entity.add_child(property)
	entity.use_skin_for(property)
...
func spawn_starting_items():
	await get_tree().create_timer(0.25).timeout
	spawn_entity_by_name("meat")
